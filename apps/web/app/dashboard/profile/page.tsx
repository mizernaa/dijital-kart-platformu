...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-2xl">
      ...
      {/* Sekmeler — scrollable */}
      <div className="flex gap-1 mb-6 bg-gray-100 p-1 rounded-xl overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-all whitespace-nowrap shrink-0 ${tab === t.id ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
            {t.icon}{t.label}
          </button>
        ))}
      </div>
        ...
        <div className="card p-6 space-y-4">
          ...
          <div className="space-y-2 pt-2 border-t border-gray-100">
            <div className="flex flex-col sm:flex-row gap-2">
              <input className="input flex-1 min-w-0" value={newProject.title} onChange={e => setNewProject(p => ({ ...p, title: e.target.value }))} placeholder="Proje adı *" />
              <input className="input sm:w-32" value={newProject.category} onChange={e => setNewProject(p => ({ ...p, category: e.target.value }))} placeholder="Kategori" />
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <input className="input flex-1 min-w-[150px]" value={newProject.tags} onChange={e => setNewProject(p => ({ ...p, tags: e.target.value }))} placeholder="Etiketler (virgülle ayır)" />
              ...
              <button onClick={() => { if (newProject.title) { setProjects(p => [...p, newProject]); setNewProject({ title: '', category: '', desc: '', tags: '', color: '#d4a843' }) } }} className="btn-primary px-3 shrink-0"><Plus size={16} /></button>
            </div>
          </div>
